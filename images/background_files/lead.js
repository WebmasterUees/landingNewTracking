(function () {
  ...
  function getScriptConfig() {
    ...
    return {
      ...
      title: clean(dataset.title || 'Formulario de admision'),
      submitUrl: clean(dataset.submitUrl || (baseUrl + '/widgets/lead/submit')),
      source: clean(dataset.source || 'widget_form'),
  ...
  function injectStyles() {
    ...
    style.textContent = '' +
      '.dm-lead-widget{font-family:Verdana,sans-serif;color:#101828;max-width:460px;border:1px solid #d0d5dd;border-radius:14px;background:#fff;box-shadow:0 10px 24px rgba(16,24,40,.08);padding:16px;}' +
      '.dm-lead-widget h3{margin:0 0 12px;font-size:20px;line-height:1.2;}' +
      '.dm-lead-grid{display:grid;grid-template-columns:1fr 1fr;gap:10px;}' +
      '.dm-lead-grid .full{grid-column:1/-1;}' +
      '.dm-lead-widget label{display:block;font-size:12px;font-weight:700;margin-bottom:4px;}' +
      '.dm-lead-widget input,.dm-lead-widget select{width:100%;padding:10px;border:1px solid #d0d5dd;border-radius:10px;font-size:14px;box-sizing:border-box;}' +
      '.dm-lead-actions{display:flex;gap:8px;margin-top:12px;}' +
      '.dm-lead-btn,.dm-wa-open{border:0;border-radius:10px;padding:11px 14px;font-weight:700;cursor:pointer;background:#821436 !important;color:#fff !important;appearance:none;-webkit-appearance:none;transition:background .18s ease,transform .08s ease,box-shadow .18s ease;}' +
      '.dm-wa-open{border-radius:999px;padding:12px 16px;}' +
      '.dm-lead-btn:hover,.dm-wa-open:hover{background:#6e112e !important;}' +
      '.dm-lead-btn:active,.dm-wa-open:active{background:#580d25 !important;transform:translateY(1px);}' +
      '.dm-lead-btn:focus-visible,.dm-wa-open:focus-visible{outline:none;box-shadow:0 0 0 3px rgba(130,20,54,.28);}' +
      '.dm-lead-btn.secondary{background:#111827;}' +
      '.dm-lead-status{font-size:13px;font-weight:700;margin-top:10px;min-height:18px;}' +
      '.dm-lead-status.error{color:#b42318;}' +
      '.dm-lead-status.ok{color:#067647;}' +
      '.dm-wa-float{position:fixed;right:18px;bottom:18px;z-index:99999;}' +
      '.dm-wa-card{display:none;width:min(360px,92vw);margin-top:10px;border:1px solid #d0d5dd;border-radius:14px;background:#fff;box-shadow:0 10px 24px rgba(16,24,40,.08);padding:14px;}' +
      '.dm-wa-form h4{margin:2px 0 12px;font-size:18px;line-height:1.2;}' +
      '.dm-wa-form input{width:100%;margin:0 0 10px;padding:10px 12px;border:1px solid #d0d5dd;border-radius:10px;font-size:14px;box-sizing:border-box;}' +
      '.dm-wa-actions{display:flex;gap:8px;margin-top:4px;}' +
      '.dm-wa-actions button{flex:1;border:0;border-radius:10px;padding:10px;font-weight:700;cursor:pointer;}' +
      '.dm-wa-close{background:#111827 !important;color:#fff !important;}' +
      '.dm-wa-success{display:none;text-align:center;padding:18px 10px;color:#067647;font-weight:700;}' +
      '@media (max-width:640px){.dm-lead-grid{grid-template-columns:1fr;}}';
  ...
  function buildWhatsappForm(config) {
    return '' +
      ...
      '  <button type="button" class="dm-wa-open">Escribenos por WhatsApp</button>' +
